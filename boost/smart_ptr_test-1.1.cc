...
void test()
{
    ...
    std::cout << "test scoped_ptr with a built-in type\n" ;
    //  test scoped_ptr with a built-in type
    long * lp = new long;
    boost::scoped_ptr<long> sp ( lp );
    std::cout << "BOOST_TEST( sp.get() == lp );\n" ;
    BOOST_TEST( sp.get() == lp );
    std::cout << "BOOST_TEST( lp == sp.get() );\n" ;
    BOOST_TEST( lp == sp.get() );
    std::cout << "BOOST_TEST( &*sp == lp );\n" ;
    BOOST_TEST( &*sp == lp );

    *sp = 1234568901L;
    std::cout << "BOOST_TEST( *sp == 1234568901L );\n" ;
    BOOST_TEST( *sp == 1234568901L );
    std::cout << "BOOST_TEST( *lp == 1234568901L );\n" ;
    BOOST_TEST( *lp == 1234568901L );
    std::cout << "ck( static_cast<long*>(sp.get()), 1234568901L );\n" ;
    ck( static_cast<long*>(sp.get()), 1234568901L );
    std::cout << "ck( lp, *sp );\n" ;
    ck( lp, *sp );

    sp.reset();
    std::cout << "BOOST_TEST( sp.get() == 0 );\n" ;
    BOOST_TEST( sp.get() == 0 );

    std::cout << "test scoped_ptr with a user defined type\n" ;
    //  test scoped_ptr with a user defined type
    boost::scoped_ptr<UDT> udt_sp ( new UDT( 999888777 ) );
    std::cout << "BOOST_TEST( udt_sp->value() == 999888777 );\n"  ;
    BOOST_TEST( udt_sp->value() == 999888777 );
    udt_sp.reset();
    udt_sp.reset( new UDT( 111222333 ) );
    std::cout << "BOOST_TEST( udt_sp->value() == 111222333 );\n" ;
    BOOST_TEST( udt_sp->value() == 111222333 );
    udt_sp.reset( new UDT( 333222111 ) );
    std::cout << "BOOST_TEST( udt_sp->value() == 333222111 );\n" ;
    BOOST_TEST( udt_sp->value() == 333222111 );

    std::cout << "test scoped_array with a build-in type\n" ;
    //  test scoped_array with a build-in type
    char * sap = new char [ 100 ];
    boost::scoped_array<char> sa ( sap );
    std::cout << "BOOST_TEST( sa.get() == sap );\n" ;
    BOOST_TEST( sa.get() == sap );
    std::cout << "BOOST_TEST( sap == sa.get() );\n" ;
    BOOST_TEST( sap == sa.get() );

    strcpy( sa.get(), "Hot Dog with mustard and relish" );
    std::cout << "BOOST_TEST( strcmp( sa.get(), \"Hot Dog with mustard and relish\" ) == 0 );\n";
    BOOST_TEST( strcmp( sa.get(), "Hot Dog with mustard and relish" ) == 0 );
    std::cout << "BOOST_TEST( strcmp( sap, \"Hot Dog with mustard and relish\" ) == 0 );\n";
    BOOST_TEST( strcmp( sap, "Hot Dog with mustard and relish" ) == 0 );

    std::cout << "BOOST_TEST( sa[0] == 'H' );\n" ;
    BOOST_TEST( sa[0] == 'H' );
    std::cout << "BOOST_TEST( sa[30] == 'h' );\n" ;
    BOOST_TEST( sa[30] == 'h' );

    sa[0] = 'N';
    sa[4] = 'd';
    std::cout << "BOOST_TEST( strcmp( sap, \"Not dog with mustard and relish\" ) == 0 );\n" ;
    BOOST_TEST( strcmp( sap, "Not dog with mustard and relish" ) == 0 );

    sa.reset();
    std::cout << "BOOST_TEST( sa.get() == 0 );\n" ;
    BOOST_TEST( sa.get() == 0 );

    std::cout << "test shared_ptr with a built-in type\n" ;
    ...
    *cp =  98765;
    BOOST_TEST( *cp == 98765 );
    *cp3 = 87654;
    std::cout << "BOOST_TEST( *cp3 == 87654 );\n" ;
    BOOST_TEST( *cp3 == 87654 );
    BOOST_TEST( *cp2 == 87654 );
    cp.swap( cp3 );
    std::cout << "BOOST_TEST( *cp == 87654 );\n" ;
    BOOST_TEST( *cp == 87654 );
    std::cout << "BOOST_TEST( *cp2 == 87654 );\n" ;
    BOOST_TEST( *cp2 == 87654 );
    std::cout << "BOOST_TEST( *cp3 == 98765 );\n" ;
